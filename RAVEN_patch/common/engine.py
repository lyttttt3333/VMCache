...
class BaseEngine:
    ...
    def backward(self, loss: torch.Tensor) -> None:
        ...
        if os.environ.get("RAVEN_LOG_BACKWARD_RESOURCES", "0") == "1" and self.iter == 0:
            status = Path("/proc/self/status").read_text()
            threads = next(
                (line.split(":", 1)[1].strip() for line in status.splitlines() if line.startswith("Threads:")),
                "unknown",
            )
            cgroup_stats = {}
            pids_cgroup = None
            for line in Path("/proc/self/cgroup").read_text().splitlines():
                ...
                if "pids" in controllers.split(","):
                    pids_cgroup = Path("/sys/fs/cgroup/pids") / relative.lstrip("/")
                    break
            for name in ("pids.current", "pids.max"):
                path = pids_cgroup / name if pids_cgroup is not None else Path("/missing")
                cgroup_stats[name] = path.read_text().strip() if path.exists() else "unavailable"
            logger.info(
                "Backward resource probe: rank=%d pid=%d threads=%s cgroup_pids=%s",
                ops.get_rank(),
                os.getpid(),
                threads,
                cgroup_stats,
            )
        if os.environ.get("RAVEN_DISABLE_AUTOGRAD_MULTITHREADING", "0") == "1":
            with torch.autograd.set_multithreading_enabled(False):
                loss.backward()
        else:
            loss.backward()
        if os.environ.get("RAVEN_LOG_BACKWARD_RESOURCES", "0") == "1" and self.iter == 0:
            logger.info("First backward complete: rank=%d", ops.get_rank())
    ...
    def optimize(self, names: list[str] | None = None) -> dict[str, float]:
        ...
        for name, model in trained.items():
            ...
            track_updates = os.environ.get("RAVEN_LOG_PARAM_UPDATES", "0") == "1"
            ...
            if track_updates:
                with torch.no_grad():
                    for parameter in model.parameters():
                        ...
                        if local.numel() == 0:
                            continue
                        snapshots.append((local, local.detach().reshape(-1)[:8].float().clone()))
            ...
            if snapshots:
                with torch.no_grad():
                    deltas = torch.cat(
                        [
                            (local.detach().reshape(-1)[: before.numel()].float() - before).abs()
                            for local, before in snapshots
                        ]
                    )
                    totals = torch.stack(
                        [
                            deltas.sum(),
                            deltas.new_tensor(deltas.numel()),
                            deltas.ne(0).sum().to(dtype=deltas.dtype),
                        ]
                    )
                    max_abs = deltas.max()
                    ops.all_reduce_sum(totals)
                    ops.all_reduce_max(max_abs)
                norm_dict[f"param_update/{name}_sampled_mean_abs"] = float(
                    (totals[0] / totals[1]).item()
                )
                norm_dict[f"param_update/{name}_sampled_global_max_abs"] = float(
                    max_abs.item()
                )
                norm_dict[f"param_update/{name}_sampled_changed_fraction"] = float(
                    (totals[2] / totals[1]).item()
                )
    ...
    def process_due(
        ...
        *,
        ...
        # At a shared save/validation boundary, commit the checkpoint only
        # after validation succeeds. A failed validation then resumes from the
        # prior checkpoint and retries this logical step instead of skipping it.
