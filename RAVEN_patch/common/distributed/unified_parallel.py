...
def init_unified_parallel(options, active_size):
    ...
    rank = dist.get_rank()
    group_backend = os.environ.get("RAVEN_UP_GROUP_BACKEND") or None
    ...
    for size in options:
        ...
        for start_rank in range(0, world_size, size):
            ...
            group = dist.new_group(ranks, backend=group_backend)
            if rank in ranks:
                local_group = group

        main_ranks = range(0, world_size, size)
        main_group = dist.new_group(main_ranks, backend=group_backend)
