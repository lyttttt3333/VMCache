...
_MESHES: dict[tuple[tuple[int, int], str], Any] = {}


def _get_hsdp_mesh(shape: tuple[int, int], backend: str = "cpu:gloo,cuda:nccl") -> DeviceMesh:
    """Create or reuse a 2D CUDA mesh without splitting the composite default PG."""
    cache_key = (shape, backend)
    if cache_key in _MESHES:
...
class DefaultPlacement:
        ...
        if "fsdp" in self.config:
            ...
            if weight_dtype is not None:
                for param_name, param in model.named_parameters():
                    if (
                        (param_name in trainable or cast_frozen)
    ...
    def _shard(
        ...
        if shard_size < world_size:
            ...
            group_backend = config.get("group_backend", "cpu:gloo,cuda:nccl")
            kwargs["mesh"] = _get_hsdp_mesh(shape, backend=group_backend)
            ...
            if class_name in wrap_classes:
                ...
                if weight_dtype is not None:
                    ...
                    for local_name, param in module.named_parameters():
                        if (
                            (f"{prefix}.{local_name}" in trainable or cast_frozen)
        ...
        if weight_dtype is not None:
            target = "floating params" if cast_frozen else "trainable params"
            logger.info("[%s] cast %d %s to %s during shard", model_name, cast_count, target, weight_dtype)
        if cpu_offload:
