...
def _configure_torch_thread_limits() -> None:
    """Apply opt-in process-local thread limits before distributed setup."""
    intraop = int(os.environ.get("RAVEN_TORCH_NUM_THREADS", "0"))
    interop = int(os.environ.get("RAVEN_TORCH_NUM_INTEROP_THREADS", "0"))
    if intraop > 0:
        import torch
...
def main(argv: list[str] | None = None) -> Any:
    _configure_torch_thread_limits()
