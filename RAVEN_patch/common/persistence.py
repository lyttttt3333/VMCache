...
class ModelHandler:
    ...
    def state_dict(self) -> Any:
        return get_model_state_dict(
            ...
            options=StateDictOptions(
                ...
                cpu_offload=self.args.get("cpu_offload", True),
                ignore_frozen_params=self.args.get("ignore_frozen_params", False),
            ),
        )
    ...
    def load_state_dict(self, sd: Any) -> None:
        full_state_dict = self.args.get("full_state_dict", False)
        ignore_frozen_params = self.args.get("ignore_frozen_params", False)
        strict = self.args.get("strict", True)
        set_model_state_dict(
            model=self.leaf,
            model_state_dict=sd,
            options=StateDictOptions(
                ...
                # A trainable-only checkpoint intentionally omits the frozen
                # base model. Its adapter topology was checked above.
                strict=strict and not ignore_frozen_params,
                broadcast_from_rank0=full_state_dict,
            ),
        )

    def validate_checkpoint_metadata(self, metadata: Any) -> None:
        """Strictly compare a trainable-only model topology before DCP load.

        DCP builds the load state dict from the current model, so keys that
        exist only in the checkpoint are otherwise invisible to
        ``load_state_dict``. ``planner_data`` retains both sides of that map and
        lets us enforce an exact adapter-key match without requiring frozen
        base weights in the checkpoint.
        """
        if not (
            self.args.get("strict", True)
            and self.args.get("ignore_frozen_params", False)
        ):
            return
        planner_data = metadata.planner_data
        if not isinstance(planner_data, dict):
            raise RuntimeError(
                f"checkpoint for '{self.path}' has no planner_data; cannot "
                "strictly validate its trainable-only topology"
            )
        expected = self.state_dict()
        checkpoint_storage_keys = {
            logical_path[1]: storage_key
            for storage_key, logical_path in planner_data.items()
            if (
                isinstance(logical_path, tuple)
                and len(logical_path) == 2
                and logical_path[0] == self.path
            )
        }
        checkpoint_keys = set(checkpoint_storage_keys)
        ...
        for key in sorted(expected_keys & checkpoint_keys):
            storage_metadata = metadata.state_dict_metadata.get(
                checkpoint_storage_keys[key]
            )
            checkpoint_size = getattr(storage_metadata, "size", None)
            expected_size = getattr(expected[key], "shape", None)
            if (
                checkpoint_size is not None
                and expected_size is not None
                and tuple(checkpoint_size) != tuple(expected_size)
            ):
                shape_mismatches.append(
                    (key, tuple(checkpoint_size), tuple(expected_size))
                )
        if missing or unexpected or shape_mismatches:
            raise RuntimeError(
                f"LoRA checkpoint topology mismatch for '{self.path}': "
...
class Checkpointer:
    ...
    def load(self, state: dict[str, Any], step: int | None = None, metadata: dict[str, Any] | None = None, checkpoints_dir: str | Path | None = None) -> dict[str, Any]:
        ...
        assert "rng" not in entries, "'rng' is reserved for the checkpointer-injected RNG state"
        entries["rng"] = RngStateHandler()
        checkpoint_metadata = dcp.FileSystemReader(str(checkpoint_dir)).read_metadata()
        for handler in entries.values():
            validate_metadata = getattr(handler, "validate_checkpoint_metadata", None)
