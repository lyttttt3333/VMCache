...
class ShardedDCPWeights(PlacementPlugin):
    ...
    def after_placement(self, state: dict[str, Any]) -> dict[str, Any]:
        ...
        if set(sharded) != set(fqn_map):
            missing_after = sorted(set(fqn_map) - set(sharded))
            added_after = sorted(set(sharded) - set(fqn_map))
            if missing_after:
                raise AssertionError(
                    "model state keys changed during shard materialization; "
                    f"missing_after={len(missing_after)} first={missing_after[:8]} "
                    f"added_after={len(added_after)} first={added_after[:8]}"
                )
            if added_after:
                logger.info(
                    "[%s] ignoring %d materialization-added state aliases before DCP load "
                    "(first: %s)",
                    state["name"],
                    len(added_after),
                    added_after[:3],
                )
        fqn_map = _build_fqn_map(sharded.keys(), self.config)
        archive = {
            fqn_map[live_fqn]: tensor
            for live_fqn, tensor in sharded.items()
            if fqn_map[live_fqn] in stored
        }
        assert archive, f"[{state['name']}] no materialized live tensors match DCP {path}"
