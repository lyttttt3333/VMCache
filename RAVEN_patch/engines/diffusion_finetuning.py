...
Config shape (engine-owned keys)::
    ...
    engine:
      ...
      algorithm_batch_size: 16                     # optional: derive ga_steps from SP groups
      up_size:
        train: 2
        validation: 8
...
class DiffusionFinetuning(BaseEngine):
    ...
    def __init__(self, config: Any, meta_model: Any):
        ...
        if "algorithm_batch_size" in self.engine_config:
            algorithm_batch_size = int(self.engine_config.algorithm_batch_size)
            if algorithm_batch_size <= 0:
                raise ValueError("engine.algorithm_batch_size must be positive")
            if algorithm_batch_size % self.num_sp_groups != 0:
                raise ValueError(
                    "engine.algorithm_batch_size must be divisible by the number "
                    f"of SP groups: {algorithm_batch_size} vs {self.num_sp_groups}"
                )
            ga_steps = algorithm_batch_size // self.num_sp_groups
            if ga_steps <= 0:
                raise ValueError(
                    "engine.algorithm_batch_size is smaller than concurrent SP groups"
                )
            self.engine_config.ga_steps = ga_steps
            logger.info(
                "Derived ga_steps=%d from algorithm_batch_size=%d and num_sp_groups=%d",
                ga_steps,
                algorithm_batch_size,
                self.num_sp_groups,
            )
        else:
