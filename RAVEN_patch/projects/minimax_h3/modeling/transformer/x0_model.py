...
class MiniMaxH3X0Model(torch.nn.Module):
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        if clean_timesteps_by_tag is None:
            clean_timesteps_by_tag = (
                MINIMAX_H3_VIDEO_CLEAN_TIMESTEP,
                MINIMAX_H3_TEXT_CLEAN_TIMESTEP,
                MINIMAX_H3_AUDIO_CLEAN_TIMESTEP,
            )
        if len(clean_timesteps_by_tag) != 3:
            raise ValueError(
                "clean_timesteps_by_tag must provide video/text/audio values, "
                f"got {clean_timesteps_by_tag!r}"
            )
        clean_by_tag = repo_t.new_tensor(clean_timesteps_by_tag)
